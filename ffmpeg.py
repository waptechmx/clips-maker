...
def create_video(text_verse, text_source, text_source_font, text_source_for_image, video_file: str, audio_file,
    ...
    result = subprocess.run(
        ['C:\\Users\\Dell\\ffmpeg\\bin\\ffprobe.exe', '-v', 'error', '-show_entries', 'stream=width,height', '-of', 'csv=p=0:s=x', video_file],
        stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    video_size = re.findall('\d+', result.stdout.decode())[0:2]
    video_width, video_height = map(int, video_size)

    # Get video duration
    ffprobe_command = f'C:\\Users\\Dell\\ffmpeg\\bin\\ffprobe.exe -i "{video_file}" -show_entries format=duration -v quiet -of csv="p=0"'
    ...
    ffmpeg_command = (f'C:\\Users\\Dell\\ffmpeg\\bin\\ffmpeg.exe -loglevel error -stats -y -loop 1 -i "{image_file}" -i "{audio_file}" '
                      ...
                      f'[v1]drawtext=fontfile=\'{text_source_font}\':text=\'{text_source}\':x=(w-text_w)/2:y={text2_y}:fontsize=70:fontcolor={font_color}:'
                      f'enable=\'between(t,{text_start_time},{video_duration})\'[v2]; '
